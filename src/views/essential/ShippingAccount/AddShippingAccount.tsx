...
import { ShippingAccounType } from '@/services/essential/shippingAccount/shippingAccountModel'
...
const AddShippingAccount: React.FC<AddShippingAccountProps> = ({
  ...
  useEffect(() => {
    ...
    if (currentRow) {
      ...
      form.resetFields()
      setInitialValues({ accountType: 'search' })
      form.setFieldsValue({ accountType: 'search' })
    }
  }, [visible, view])
          ...
          >
            ...
            {item.formType === 'select' && (
              <Select
                ...
                filterOption
                options={(carrierOptions || []).map((item) => ({
                  label: item.id,
                  value: item.name,
                }))}
              />
